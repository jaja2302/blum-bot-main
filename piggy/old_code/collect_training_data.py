...
class HoopTracker:
    def __init__(self):
        ...
        # Konfigurasi path Tesseract jika belum diset di environment variables
        pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'  # Sesuaikan dengan path instalasi Anda
    ...
    def detect_score(self, screenshot):
        """Deteksi skor menggunakan OCR"""
        height = screenshot.shape[0]
        width = screenshot.shape[1]
        
        # Crop bagian atas dimana skor biasanya muncul
        score_region = screenshot[0:int(height*0.15), int(width*0.4):int(width*0.6)]
        
        # Convert ke grayscale
        gray = cv2.cvtColor(score_region, cv2.COLOR_BGR2GRAY)
        
        # Threshold untuk mendapatkan teks yang lebih jelas
        _, thresh = cv2.threshold(gray, 170, 255, cv2.THRESH_BINARY)
        
        # OCR untuk mendapatkan teks
        try:
            ...
            if score is not None and score != self.current_score:
                self.last_score = self.current_score
                self.current_score = score
                print(f"Score updated: {self.current_score}")
    ...
    def detect_score_and_time(self, screenshot):
        ...
        score_region = screenshot[0:int(height*0.1), int(width*0.1):int(width*0.2)]  # Area merah di kiri
        time_region = screenshot[0:int(height*0.1), int(width*0.4):int(width*0.6)]   # Area tengah untuk waktu
    ...
    def learn_from_game(self):
        ...
        try:
            while True:
                ...
                # Detect hoop, score, and time
                hoop_pos = self.detect_hoop(screenshot)
                score, game_time = self.detect_score_and_time(screenshot)
                ...
                cv2.putText(screenshot, f"Missed: {self.missed_detections}", (10, 180), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                cv2.putText(screenshot, f"Score: {score}", (10, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                cv2.putText(screenshot, f"Time: {game_time}", (width - 150, 30), 
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
